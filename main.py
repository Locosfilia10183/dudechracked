...
def get_imap_server(email):
    ...
    outlook = ["hotmail.com", "outlook.com", "live.com", "hotmail.fr", "outlook.fr", "live.fr"]
...
def start():
    ...
    if checking:
        return jsonify({'status': 'error', 'message': 'Check läuft bereits!'})
    ...
    if mode == 'inbox' and not keyword:
        return jsonify({'status': 'error', 'message': 'Bitte Keyword eingeben!'})
    ...
    if not accounts:
        return jsonify({'status': 'error', 'message': 'Keine gültigen Accounts!'})
    ...
    return jsonify({'status': 'ok', 'message': f'Gestartet! {len(accounts)} Accounts'})
